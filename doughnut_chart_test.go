	...
	for i, tt := range tests {
		...
		if tt.themed {
			t.Run(strconv.Itoa(i)+"-"+tt.name+"-painter", func(t *testing.T) {
				...
				validateDoughnutChartRender(t, p, rp, tt.makeOptions(), tt.svg, tt.pngCRC)
			})
			t.Run(strconv.Itoa(i)+"-"+tt.name+"-options", func(t *testing.T) {
				...
				validateDoughnutChartRender(t, p, rp, opt, tt.svg, tt.pngCRC)
			})
		} else {
...
func validateDoughnutChartRender(t *testing.T, svgP, pngP *Painter, opt DoughnutChartOption, expectedSVG string, expectedCRC uint32) {
	t.Helper()
