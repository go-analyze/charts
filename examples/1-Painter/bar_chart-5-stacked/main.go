...
func writeFile(buf []byte) error {
	...
	file := filepath.Join(tmpPath, "bar-chart-5-stacked.png")
...
func main() {
	...
	opt.SeriesList[1].MarkPoint.AddGlobalPoints(charts.SeriesMarkTypeMax)
	opt.SeriesList[1].MarkPoint.ValueFormatter = func(f float64) string {
		return "Sum:" + charts.FormatValueHumanizeShort(f, 0, false)
	}
